...
<nav role="navigation"
  ...
  class="flex items-center justify-between p-4 border-t select-none border-gary-600 dark:border-gray-600 sm:px-6">
  <div class="flex justify-start flex-1 sm:hidden">
    @if ($paginator->onFirstPage())
    {{-- previous disable --}}
    <span
      class="relative inline-flex items-center px-4 py-2 text-sm font-medium leading-5 text-gray-800 bg-white border border-gray-300 rounded dark:text-gray-400 dark:bg-secondary-700 dark:border-secondary-600">
      {!! __('pagination.previous') !!}
    </span>
    @else
    {{-- previous enable --}}
    <a href="{{ $paginator->previousPageUrl() }}"
      class="relative inline-flex items-center px-4 py-2 text-sm font-medium leading-5 transition bg-white border border-gray-300 rounded dark:text-gray-600 dark:border-secondary-600 hover:text-gray-600 dark:hover:text-gray-600 focus:outline-none focus:border-primary-300 focus:ring focus:ring-primary-300 focus:ring-opacity-30 dark:bg-secondary-700 dark:focus:ring-primary-500 dark:focus:ring-opacity-30">
      {!! __('pagination.previous') !!}
    </a>
    @endif
    {{-- next enable --}}
  ...
  <div class="flex-col hidden lg:flex-row sm:flex-1 sm:flex sm:items-center sm:justify-between">
    <div>
      <p class="text-sm leading-5 dark:text-gray-800">
    ...
    <div>
      <span class="relative z-0 inline-flex mt-2 shadow-sm lg:mt-0">
        ...
        <span aria-disabled="true"
          ...
          <span
            class="relative inline-flex items-center px-2 py-2 text-sm font-medium leading-5 text-gray-400 bg-white border border-gray-600 rounded-l dark:text-gray-600 dark:border-secondary-600 dark:bg-secondary-700"
            aria-hidden="true">
            <svg class="w-5 h-5"
              fill="currentColor"
              viewBox="0 0 20 20">
        ...
        <a href="{{ $paginator->previousPageUrl() }}"
          rel="prev"
          class="relative inline-flex items-center px-2 py-2 text-sm font-medium leading-5 transition bg-white border border-gray-400 rounded-l hover:text-gray-800 focus:z-10 focus:outline-none focus:border-primary-300 focus:ring focus:ring-primary-300 focus:ring-opacity-30 dark:focus:ring-primary-500 dark:focus:ring-opacity-30 dark:text-gray-300 dark:border-secondary-600 dark:bg-secondary-700"
          aria-label="{{ __('pagination.previous') }}">
          <svg class="w-5 h-5"
            fill="currentColor"
            viewBox="0 0 20 20">
